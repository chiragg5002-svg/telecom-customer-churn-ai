...
const {
  ...
  Header, Footer, PageNumber, TableOfContents, ExternalHyperlink, VerticalAlign,
} = require("docx");

const ROOT = __dirname;
const IMG = path.join(ROOT, "report_images");
const model = JSON.parse(fs.readFileSync(path.join(ROOT, "model", "model_metrics.json")));
...
function figure(file, caption, maxWidth = 560) {
  ...
  return [
    new Paragraph({
      ...
      children: [new ImageRun({ data: fs.readFileSync(path.join(IMG, file)), transformation: { width, height }, type: "png" })],
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 },
...
function pRuns(runs) { return new Paragraph({ spacing: { after: 160 }, children: runs }); }
...
function cell(text, opts = {}) {
  ...
  return new TableCell({
    ...
    shading: shade ? { type: ShadingType.CLEAR, color: "auto", fill: shade } : (header ? { type: ShadingType.CLEAR, color: "auto", fill: "1F3864" } : undefined),
    verticalAlign: VerticalAlign.CENTER,
    margins: { top: 80, bottom: 80, left: 120, right: 120 },
    children: [new Paragraph({
      children: [new TextRun({ text: String(text), bold: header, color: header ? "FFFFFF" : "000000", size: header ? 20 : 20 })],
    })],
  });
}
...
function table(headerRow, rows, colWidths) {
  ...
  return new Table({
    ...
    rows: [
      new TableRow({ tableHeader: true, children: headerRow.map((t, i) => cell(t, { header: true, width: colWidths[i] })) }),
...
const titlePage = [
  new Paragraph({ spacing: { before: 2400 }, alignment: AlignmentType.CENTER,
    children: [new TextRun({ text: "Telecom Customer Churn", bold: true, size: 56 })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 400 },
    children: [new TextRun({ text: "A Decision Support Dashboard", size: 32, color: "444444" })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 800 },
    children: [new TextRun({ text: "IBM SkillsBuild Data Analytics with AI Academic Internship", size: 24 })] }),
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 1200 },
    children: [new TextRun({ text: "Prepared by: Hari", size: 22, bold: true })] }),
  new Paragraph({ alignment: AlignmentType.CENTER,
    children: [new TextRun({ text: "Final-year B.E., Artificial Intelligence and Data Science", size: 20, color: "555555" })] }),
  new Paragraph({ alignment: AlignmentType.CENTER,
    children: [new TextRun({ text: "Srinivas Institute of Technology (VTU), Mangaluru", size: 20, color: "555555" })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 1200 },
    children: [new TextRun({ text: new Date().toISOString().slice(0, 10), size: 20, color: "777777" })] }),
...
const abstract = [
  h1("Abstract"),
  p("Telecom operators typically lose roughly a quarter of their customer base every reporting " +
...
const introduction = [
  h1("1. Introduction"),
  p("Customer churn — a customer discontinuing service — is one of the most direct threats to " +
...
const problemStatement = [
  ...
  p("A telecom operator has no systematic way to know which of its currently active customers " +
  ...
  p("In this dataset, 26.5% of customers have already churned, representing an estimated " +
  ...
  p("Manually reviewing thousands of customer billing and service records to judge who might " +
  ...
  p("The Telco Customer Churn dataset provides 7,043 customer records with demographic, " +
  ...
  p("Exploratory data analysis quantifies which customer segments churn most (by contract, " +
    "tenure, internet service and payment method), turning anecdote into a ranked, evidenced " +
    "set of risk factors."),
  h2("How can AI/ML help?"),
  p("A supervised classification model combines all available fields simultaneously to produce " +
  ...
  p("A trained and evaluated churn model, a scored dataset covering all 7,043 customers, a " +
...
const objectives = [
  h1("3. Objectives"),
  bullet("Understand and document the dataset: fields, size, target balance and data quality"),
  bullet("Clean the data and justify every cleaning decision taken"),
  bullet("Perform exploratory data analysis to identify churn patterns and drivers"),
  bullet("Engineer features that summarise service bundling and payment behaviour"),
  bullet("Train and compare multiple classification models against a validated baseline"),
  bullet("Evaluate the selected model with metrics appropriate for imbalanced classification"),
  bullet("Create meaningful, purposeful visualisations for each stage of the analysis"),
  bullet("Build an interactive dashboard organised around decisions, not just data"),
...
const scope = [
  h1("4. Scope"),
  p("In scope: a batch-trained binary churn classifier; a rule-based insight engine computed " +
  ...
  p("Out of scope: real-time streaming inference, an LLM-generated narrative layer, and causal " +
...
const datasetDescRows = [
  ...
  ["Source", "IBM Cloud Pak for Data sample datasets (IBM telco-customer-churn-on-icp4d GitHub repository)"],
  ...
  ["Missing values", "0 explicit nulls; 11 blank TotalCharges strings, resolved during cleaning"],
...
const datasetDescription = [
  ...
  bullet("Account: tenure, Contract, PaperlessBilling, PaymentMethod, MonthlyCharges, TotalCharges"),
  bullet("Services: PhoneService, MultipleLines, InternetService, OnlineSecurity, OnlineBackup, " +
    "DeviceProtection, TechSupport, StreamingTV, StreamingMovies"),
];

// ------------------------------------------------------------- CLEANING ----
const cleaningRows = model.cleaning_log.map((l) => [l.step, String(l.found), l.action, l.reason]);
const dataCleaning = [
  h1("6. Data Cleaning"),
  p("Each step below is applied programmatically by src/preprocessing.clean_data() and its " +
    "log is captured automatically - the table is not hand-typed."),
  table(["Step", "Found", "Action taken", "Reason"], cleaningRows, [2000, 900, 2600, 3900]),
  h3("Key decisions explained"),
  bullet("TotalCharges is stored as text with 11 blank values; every one of the 11 belongs to a " +
    "customer with tenure = 0 (not yet billed), so they were set to 0.0 rather than dropped."),
  bullet("\u201CNo internet service\u201D / \u201CNo phone service\u201D categories were folded into " +
  ...
  bullet("SeniorCitizen was recoded from 0/1 to No/Yes so every binary field shares one encoding."),
  bullet("No rows were removed: 7,043 in, 7,043 out. The IQR outlier check on tenure, monthly " +
...
const eda = [
  ...
  p("Nine charts were produced to understand distributions, trends and relationships in the " +
  ...
  p("Observation: 26.5% of customers have churned - a meaningful minority class. Insight: the " +
  ...
  ...figure("02_churn_by_contract.png", "Figure 2. Churn rate by contract type."),
  ...figure("03_churn_by_tenure.png", "Figure 3. Churn rate by tenure group."),
  p("Observation: churn falls sharply as contract length and tenure increase - from 42.7% " +
  ...
  ...figure("04_tenure_distribution.png", "Figure 4. Tenure distribution by churn outcome.", 420),
  ...figure("05_monthly_charges_distribution.png", "Figure 5. Monthly charges distribution by churn outcome.", 420),
  ...figure("06_churn_by_internet.png", "Figure 6. Churn rate by internet service type."),
  p("Observation: fibre-optic customers churn more than twice as often as DSL customers " +
  ...
  ...figure("07_churn_by_payment.png", "Figure 7. Churn rate by payment method.", 500),
  p("Observation: electronic-check payers churn at 45.3% versus 16.0% for customers paying by " +
  ...
  ...figure("08_churn_by_protection.png", "Figure 8. Churn rate by number of protection/support services (internet customers)."),
  p("Observation: internet customers with none of the four protection/support services churn " +
  ...
  ...figure("09_correlation_heatmap.png", "Figure 9. Correlation heatmap of numeric and encoded features.", 480),
  p("Observation: Churn correlates most strongly with Contract (negative - longer contract, " +
...
const insightsSection = [
  ...
  p("Beyond the EDA charts above, the cleaned and scored dataset was queried directly for " +
  ...
  bullet("Data evidence: month-to-month customers churn at 42.7%, one-year at 11.3%, two-year at 2.8%"),
  bullet("Observation: churn falls monotonically and steeply as contract length increases"),
  bullet("Insight: contract length is one of the strongest observable separators of leavers and stayers"),
  bullet("Implication: this may represent a retention lever worth testing - moving suitable " +
...
const mlMethodology = [
  ...
  h3("Pipeline"),
  bullet("Train/test split: 80% / 20%, stratified by target, random_state = 42 " +
    `(${model.train_rows.toLocaleString()} train / ${model.test_rows.toLocaleString()} test rows)`),
  bullet("Preprocessing: StandardScaler on numeric features, OneHotEncoder on categorical features, inside a scikit-learn Pipeline"),
  bullet("Candidates compared: Logistic Regression, Random Forest, Gradient Boosting, each against a DummyClassifier (majority-class) baseline"),
  bullet("Validation: 5-fold stratified cross-validation on the training split, scored on ROC-AUC and PR-AUC"),
  bullet(`Model selection rule: ${model.selection_rule}`),
  bullet(`Decision threshold rule: ${model.threshold_rule} (selected threshold: ${model.decision_threshold})`),
  bullet("Every customer is finally scored using out-of-fold cross-validated predictions, so no customer is ever scored by a model that was trained on that same customer"),
];

// -------------------------------------------------------------- TRAINING ---
const comparisonRows = Object.entries(model.comparison).map(([name, r]) => [
  name,
  r.cv_roc_auc === 0.5 && name.startsWith("Baseline") ? "0.500 (by definition)" : r.cv_roc_auc.toFixed(4),
  r.test.roc_auc.toFixed(4),
  r.test.f1.toFixed(4),
]);
const modelTraining = [
  h1("10. Model Training"),
  p("Four models were trained and compared: a majority-class baseline and three classifiers, " +
    "each wrapped in the same preprocessing pipeline for a fair comparison."),
  table(["Model", "5-fold CV ROC-AUC", "Test ROC-AUC", "Test F1 (threshold 0.5)"], comparisonRows, [3000, 2300, 2000, 2100]),
  p(`Logistic Regression, Random Forest and Gradient Boosting all land within about 0.002 AUC ` +
...
const modelEvaluation = [
  ...
  p("Metrics appropriate for imbalanced binary classification are reported at both the default " +
    "0.5 threshold and the tuned threshold (0.32), on the held-out test set only."),
  table(["Metric", "Threshold = 0.50", `Threshold = ${model.decision_threshold}`], evalRows, [3200, 3200, 3200]),
  ...figure("11_roc_curves.png", "Figure 10. ROC curves - model comparison on the test set.", 380),
  ...figure("12_confusion_matrix.png", "Figure 11. Confusion matrix at the tuned threshold.", 340),
  p(`At the tuned threshold, recall rises to ${pct(T.recall)} (catching most churners) at the ` +
  ...
  ...figure("10_feature_importance.png", "Figure 12. Top 10 permutation feature importances.", 420),
  p("tenure dominates, followed by InternetService and Contract - consistent with the EDA " +
  ...
  bullet("The model uses only the fields present in this dataset - no usage minutes, complaint history, or competitor pricing"),
  bullet("It is a correlational model: features found \u201Cimportant\u201D are associated with churn, not proven causes of it"),
  bullet("Trained on this operator's historical customers; would need retraining before use on a different customer base"),
];

// ---------------------------------------------------------- DASHBOARD DESIGN
const dashboardDesign = [
  h1("12. Dashboard Design"),
  p("The dashboard follows the masterclass's five-level structure (KPIs -> Trends -> Drivers -> " +
    "Risk -> Action) across four pages, so it never becomes an undirected data dump."),
  h3("Top 3 KPIs (above the fold)"),
  bullet(`Churn rate - ${pct(0.2653698707936959)} of customers have churned - answers \u201Cis the business healthy?\u201D in one glance`),
  bullet("Monthly revenue lost to churn - quantifies the cost of the problem in dollars, not just customer count"),
  bullet("Active customers at risk (High + Medium tier) - the actionable number: who to act on now, and how much revenue is exposed"),
  h3("Page structure"),
  bullet("Page 1 - Executive Overview: the three KPIs above, supporting KPIs, churn split and risk-tier distribution"),
  bullet("Page 2 - Trend & Driver Analysis: churn trend across the customer lifecycle, a selectable driver breakdown, and model feature importances"),
  bullet("Page 3 - Predict a Customer: score any customer profile on demand, with a plain-language explanation of what pushed the score up or down"),
  bullet("Page 4 - Risk & Recommendations: the seven Risk/Opportunity findings and a downloadable, ranked retention list"),
  p("Every chart on every page answers one of the six framing questions from the masterclass " +
...
const systemArchitecture = [
  ...
  ...figure("00_architecture.png", "Figure 13. End-to-end system architecture.", 560),
  p("Data flows one direction only: the raw CSV is cleaned and explored, features are " +
...
const implementation = [
  ...
  bullet("GET /model_info - selected model, decision threshold, evaluation metrics, input schema"),
  bullet("GET /dataset - row/column counts, dtypes, missing-value count, a data sample"),
  bullet("GET /kpis - the executive-overview KPI values"),
  bullet("GET /insights - the seven Risk/Opportunity findings, computed live from the scored data"),
  bullet("POST /predict - scores one customer; validates every field, rejects impossible combinations (e.g. streaming services set to Yes with no internet service), and returns a plain-language explanation"),
  h3("Frontend - Streamlit dashboard"),
  p("Four pages as described in Section 12, reading directly from the saved model and scored " +
  ...
  bullet("Automated smoke tests on every Flask endpoint (valid input, missing fields, invalid categories, impossible combinations, malformed JSON, unknown routes) - all return the expected status codes"),
  bullet("A full clean-environment re-run of src/train_model.py reproduces identical metrics (random_state = 42 throughout)"),
  bullet("The Streamlit dashboard was launched and screenshotted headlessly across all four pages, including submitting the prediction form, to confirm it renders without exceptions"),
];

// ---------------------------------------------------------------- RESULTS --
const results = [
  h1("15. Results"),
  p(`The selected Logistic Regression model reached a test ROC-AUC of ${M.roc_auc.toFixed(3)} ` +
...
function insightBlock(id, type, title, fact, insight, implication, action, addressable) {
  const out = [
  ...
  if (addressable) out.push(new Paragraph({ spacing: { after: 200 }, children: [new TextRun({ text: addressable, italics: true, size: 20, color: "555555" })] }));
  return out;
}

const insightData = [
  ["R1", "Risk", "Month-to-month contracts hold most of the churn",
  ...
  ["R2", "Risk", "The first year is the danger zone",
  ...
  ["R3", "Risk", "Fibre optic customers churn 2.2x as often as DSL customers",
  ...
  ["R4", "Risk", "Electronic-check payers churn far more than automatic payers",
  ...
  ["O1", "Opportunity", "Protection and support services go with retention",
  ...
  ["O2", "Opportunity", "A ranked retention list is available today",
  ...
  ["O3", "Opportunity", "Long-contract customers are a stable revenue base",
...
let insightParas = [];
for (const row of insightData) insightParas = insightParas.concat(insightBlock(...row));

// -------------------------------------------------------- RISKS/OPPS/ACTIONS
const risksOppsActions = [
  h1("16. Risks, Opportunities and Recommended Actions"),
  p("Full detail for all seven findings (also served live by the /insights API endpoint and " +
...
const limitations = [
  h1("17. Limitations"),
  bullet("Single-snapshot dataset with no timestamps - trends over time (e.g. is churn rising this quarter) cannot be assessed, only cross-sectional patterns"),
  bullet("No usage-quality, complaint-log, or competitor-pricing fields exist, so the fibre-optic churn finding (R3) is flagged as a risk to investigate, not a diagnosed cause"),
  bullet("All relationships reported are associations from observational data; establishing causation would require a controlled trial (e.g. A/B-testing the protection-service offer in O1)"),
  bullet("The model is trained on this operator's historical customers and would need retraining and re-validation before use on a different customer base or after major product changes"),
];

// ------------------------------------------------------------- FUTURE SCOPE
const futureScope = [
  h1("18. Future Scope"),
  bullet("Retrain periodically as new billing cycles complete, and monitor for feature drift"),
  bullet("Run a controlled A/B trial on the top opportunity (protection-service trial) to move from correlation to a measured causal effect"),
  bullet("Extend the dataset with usage, complaint or NPS data to explain the fibre-optic churn gap"),
  bullet("Add cost-sensitive threshold tuning using actual retention-offer cost and customer lifetime value once available from finance"),
];

// -------------------------------------------------------------- CONCLUSION -
const conclusion = [
  h1("19. Conclusion"),
  p("This project takes the Telco Customer Churn dataset through the complete masterclass flow: " +
...
const references = [
  h1("20. References"),
  bullet("Telco Customer Churn dataset - IBM, https://raw.githubusercontent.com/IBM/telco-customer-churn-on-icp4d/master/data/Telco-Customer-Churn.csv"),
  bullet("Pedregosa et al., Scikit-learn: Machine Learning in Python, Journal of Machine Learning Research 12, 2011"),
  bullet("McKinney, W., Data Structures for Statistical Computing in Python, Proceedings of the 9th Python in Science Conference, 2010 (pandas)"),
  bullet("Streamlit Inc., Streamlit documentation, https://docs.streamlit.io"),
  bullet("Flask (Pallets Projects), Flask documentation, https://flask.palletsprojects.com"),
];

// ------------------------------------------------------------- APPENDIX ----
const appendix = [
  h1("21. Appendix - Dashboard Screenshots"),
  ...figure("dash_1_overview.png", "Screenshot 1. Executive Overview page.", 560),
  ...figure("dash_2_trends.png", "Screenshot 2. Trend & Driver Analysis page.", 560),
  ...figure("dash_3b_predict_result.png", "Screenshot 3. Predict a Customer page, with a scored result.", 560),
  ...figure("dash_4_risk.png", "Screenshot 4. Risk & Recommendations page.", 560),
];
...
const sections = [{
  ...
  headers: {
    default: new Header({ children: [new Paragraph({ alignment: AlignmentType.RIGHT,
      children: [new TextRun({ text: "Telecom Customer Churn - Project Report", size: 16, color: "888888" })] })] }),
  },
  footers: {
  ...
  styles: {
    ...
    paragraphStyles: [
      { id: "Heading1", name: "Heading 1", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { bold: true, size: 30, color: "1F3864" }, paragraph: { spacing: { before: 360, after: 160 } } },
      { id: "Heading2", name: "Heading 2", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { bold: true, size: 26, color: "2E5395" }, paragraph: { spacing: { before: 280, after: 120 } } },
      { id: "Heading3", name: "Heading 3", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { bold: true, size: 23, color: "444444" }, paragraph: { spacing: { before: 200, after: 100 } } },
    ],
  },
  sections,
});
...
Packer.toBuffer(doc).then((buf) => {
  fs.writeFileSync(path.join(ROOT, "Hari_ProjectReport.docx"), buf);
  console.log("wrote Hari_ProjectReport.docx", buf.length, "bytes");
});
